...
md"""
## Project 2 — Multibody kinematic modeling workflow

The goal of this notebook is to model and analyze a dual-slider mechanism using constraint-based multibody kinematics.

The approach follows a structured workflow:

1. define the generalized coordinates for each body  
2. define the mechanism geometry and coordinate frames  
3. build the constraint equations `C(q,t)=0`  
4. solve for the position of the system over one full rotation  
5. solve for the generalized velocities `q̇` and accelerations `q̈`  
6. visualize and interpret the motion of the mechanism  

This workflow connects the physical system to a numerical solution using:
- symbolic constraint formulation (`Symbolics.jl`)
- nonlinear solving (`NonlinearSolve.jl`)
- visualization and animation (`Plots.jl`)

The result is a complete kinematic description of the mechanism, including position, velocity, and acceleration behavior.
"""

# ╔═╡ 5e32e0c8-298a-439e-85ca-6e3367ddf28c
md"""
## Step 1 — Define the generalized coordinates

To model the dual-slider mechanism, we use full planar rigid-body coordinates for each moving body.

Each body has 3 generalized coordinates:
- body 1 (slider 1): `(x₁, y₁, θ₁)`
- body 2 (slider 2): `(x₂, y₂, θ₂)`
- body 3 (rigid bar): `(x₃, y₃, θ₃)`

This gives the full generalized coordinate vector

`q = [x₁, y₁, θ₁, x₂, y₂, θ₂, x₃, y₃, θ₃]`

This coordinate choice is useful because:
- it follows standard multibody kinematics notation
- it makes the constraints easy to write in vector form
- it gives a direct path to the Jacobian `C_q`

In the code below, we define the symbolic generalized coordinates that will be used to build the constraint equations.
"""

# ╔═╡ e0a2b2f8-c836-4492-9158-7357ea116dbd
begin
    @variables x1 y1 θ1 x2 y2 θ2 x3 y3 θ3 t
    q_sym = [x1, y1, θ1, x2, y2, θ2, x3, y3, θ3]
end

# ╔═╡ ea738297-5c3a-432f-8f2d-12b0cd71c97f
md"""
## Step 2 — Define the mechanism geometry and coordinate frames

Next, we define the physical geometry of the mechanism in a fixed inertial `x-y` frame.

The main assumptions are:
- slider 1 moves along the `+45°` track, so its center lies on `y = x`
- slider 2 moves along the `-45°` track, so its center lies on `y = -x`
- the rigid bar has length `L3 = 0.10 m`
- the bar angle is prescribed as `θ₃(t) = θ₃₀ + Ω t`
- the hinges connect the bar ends to the centers of the sliders

This step sets the dimensions, driver motion, and local body geometry needed to write the constraints.

The code below defines the bar length, angular speed, initial angle, local endpoint vectors, and the planar rotation matrix.
"""
...
# ╔═╡ cad92e3e-10ca-4d55-ba0a-3974a6717f26
begin
    A(θ) = [cos(θ) -sin(θ);
            sin(θ)  cos(θ)]
end

# ╔═╡ c922cd5b-1940-494c-8bd6-68c0307f9136
md"""
## Step 3 — Build the constraint equations `C(q,t)=0`

The mechanism has 9 generalized coordinates, so we need 9 scalar constraint equations.

The constraints are:
1. slider 1 remains on the `y = x` track
2. slider 2 remains on the `y = -x` track
3. slider 1 orientation is fixed by its guide rail
4. slider 2 orientation is fixed by its guide rail
5. slider 1 center coincides with one end of the bar
6. slider 2 center coincides with the other end of the bar
7. the rigid bar angle follows the prescribed driver motion

Together, these define the full constraint vector `C(q,t)=0`.

The code below builds the symbolic position vectors, forms the constraint equations, computes the Jacobians, and converts the symbolic expressions into callable Julia functions.
"""

# ╔═╡ 17829619-df43-4ff9-86a7-35cc2d7cc875
begin
    r1 = [x1, y1]
    r2 = [x2, y2]
    r3 = [x3, y3]
...
# ╔═╡ 0ca625bb-e9e7-440b-8d20-5399f96a512d
begin
    Cq_sym = Symbolics.jacobian(C_sym, q_sym)
    Ct_sym = Symbolics.jacobian(C_sym, [t])
end

# ╔═╡ 79c84257-75f8-477e-8efa-2aacc9d7f4f6
begin
    C_expr  = Symbolics.build_function(C_sym, q_sym, t; expression = Val(false))
    Cq_expr = Symbolics.build_function(Cq_sym, q_sym, t; expression = Val(false))
    Ct_expr = Symbolics.build_function(Ct_sym, q_sym, t; expression = Val(false))

    C_fun  = eval(first(C_expr))
    Cq_fun = eval(first(Cq_expr))
    Ct_fun = eval(first(Ct_expr))
end

# ╔═╡ 5444961d-881f-4375-92ec-cdf7f876d2a3
begin
    C_vec(q, t) = collect(vec(C_fun(q, t)))
    Cq_mat(q, t) = Matrix(Cq_fun(q, t))
    Ct_vec(q, t) = collect(vec(Ct_fun(q, t)))
end

# ╔═╡ 4311f69f-b0ee-43f2-a532-b16bb401d2ac
md"""
## Step 4 — Evaluate the constraints and solve for position over one full rotation

Before solving the motion, we choose an initial configuration `q0` that satisfies the constraints at `t = 0`.

Checking `norm(C_vec(q0, 0.0))` confirms whether the starting geometry is physically consistent. A value near zero means the initial pose satisfies the mechanism constraints.

After that, we solve the nonlinear system `C(q,t)=0` at each time step over one full rotation of the bar. The previous solution is used as the next initial guess, which makes the solve more stable.

The code below defines the initial condition, checks the constraint residual, and computes the full position history `q(t)`.
"""

# ╔═╡ 2fa54072-ae3f-4a7e-aa84-dd4f0385f3e5
begin
    q0 = [
        L3/2,   L3/2,   pi/4,    # slider 1 on y = x
        L3/2,  -L3/2,  -pi/4,    # slider 2 on y = -x
        L3/2,   0.0,    θ30      # bar center
    ]
end

# ╔═╡ abcce979-0920-46c6-94ab-bc8d6ff3b82a
norm(C_vec(q0, 0.0))

# ╔═╡ 8cec6ac6-b6a0-40f5-82de-a7e34950e6fc
begin
    function position_residual!(resid, q, p)
        resid .= C_vec(q, p)
        return resid
    end

    function solve_position(q_guess, t_now)
        prob = NonlinearProblem(position_residual!, q_guess, t_now)
        sol = solve(prob, NewtonRaphson(); abstol=1e-12, reltol=1e-12, maxiters=50)
        return sol.u
    end
end

# ╔═╡ 63065b71-53cf-42d6-8f72-93d2305914ec
begin
    tspan = range(0.0, π, length=181)
    q_hist = zeros(length(q0), length(tspan))
    q_hist[:, 1] .= q0

    for k in 2:length(tspan)
        q_hist[:, k] .= solve_position(q_hist[:, k-1], tspan[k])
    end
end

# ╔═╡ 3b514e25-0de4-4f97-bee2-d8184bb23e1a
md"""
## Step 5 — Solve for the generalized velocities and accelerations

Once the position history is known, the next step is to compute the generalized velocities `q̇` and accelerations `q̈`.

For velocities, we use the differentiated constraint equation

`C_q(q,t) q̇ + C_t(q,t) = 0`

and solve for `q̇` at each time step.

For accelerations, we differentiate the velocity constraint once more and solve for `q̈`. In this notebook, the remaining second-derivative terms are approximated numerically.

This step gives the full kinematic response of the mechanism:
- generalized velocities `q̇`
- generalized accelerations `q̈`

It also lets us verify that the prescribed rigid-bar motion behaves as expected:
- `θ₃̇ = 2 rad/s`
- `θ₃̈ ≈ 0`
"""
...
begin
    function solve_velocity(q_now, t_now)
        Cq = Cq_mat(q_now, t_now)
        Ct = Ct_vec(q_now, t_now)
        return Cq \ (-Ct)
    end

    qdot_hist = zeros(length(q0), length(tspan))
    for k in eachindex(tspan)
        qdot_hist[:, k] .= solve_velocity(q_hist[:, k], tspan[k])
    end
end

# ╔═╡ 412489d3-b753-4643-9937-213019d2f136
begin
    plot(tspan, qdot_hist[9, :],
        xlabel = "time (s)",
        ylabel = "θ₃ dot (rad/s)",
        title = "Rigid-bar angular velocity vs time",
        label = "θ₃̇ from constraints")
end
...
To compute `q̈`, we differentiate the velocity constraint again and solve for the generalized accelerations.

This allows us to study how the slider and bar velocities are changing through the motion, even though the rigid bar itself rotates with constant angular speed.
"""

# ╔═╡ 5b7a6470-55e3-42d2-ba5e-c85cd2af3004
begin
    function accel_rhs(q_now, qdot_now, t_now; h=1e-6)
        Cq_now = Cq_mat(q_now, t_now)
        Ct_now = Ct_vec(q_now, t_now)

        q_plus = q_now .+ h .* qdot_now
        t_plus = t_now + h

        Cq_plus = Cq_mat(q_plus, t_plus)
        Ct_plus = Ct_vec(q_plus, t_plus)

        v_now  = Cq_now * qdot_now + Ct_now
        v_plus = Cq_plus * qdot_now + Ct_plus

        gamma = (v_plus - v_now) ./ h
        return -gamma
    end

    function solve_acceleration(q_now, qdot_now, t_now)
        Cq = Cq_mat(q_now, t_now)
        rhs = accel_rhs(q_now, qdot_now, t_now)
        return Cq \ rhs
    end

    qddot_hist = zeros(length(q0), length(tspan))
    for k in eachindex(tspan)
        qddot_hist[:, k] .= solve_acceleration(q_hist[:, k], qdot_hist[:, k], tspan[k])
    end
end

# ╔═╡ 9ff6c4c3-1ef8-4ae4-a9c2-4054c484eb2e
begin
    plot(tspan, qddot_hist[9, :],
        xlabel = "time (s)",
        ylabel = "θ₃ double dot (rad/s²)",
        title = "Rigid-bar angular acceleration vs time",
        label = "θ₃̈")
end

# ╔═╡ 8380f12d-05c5-4352-8ff8-b35135fe9f0c
begin
    plot(tspan, qddot_hist[1, :],
        xlabel = "time (s)",
        ylabel = "acceleration (m/s²)",
        title = "Slider x-accelerations vs time",
        label = "x1̈")
...
begin
    plot(tspan, qddot_hist[7, :],
        xlabel = "time (s)",
        ylabel = "acceleration (m/s²)",
        title = "Bar-center acceleration vs time",
        label = "x3̈")

    plot!(tspan, qddot_hist[8, :], label = "y3̈")
end

# ╔═╡ 0529841e-9508-4e75-944a-c6079c7504b3
md"""
## Step 6 — Visualize and interpret the motion of the system

The final step is to visualize the mechanism as it moves through one full rotation.

In this section, we:
- extract the slider centers and bar endpoints from the solved coordinates
- plot the paths of the sliders and bar center
- plot selected coordinate histories versus time
- animate the mechanism motion

These figures help confirm that the constraint model is producing physically meaningful motion and make the full kinematic behavior easier to interpret.
"""
...
begin
    ...
    left_end  = [ [x3_hist[k], y3_hist[k]] + A(θ3_hist[k]) * s31 for k in eachindex(tspan) ]
    right_end = [ [x3_hist[k], y3_hist[k]] + A(θ3_hist[k]) * s32 for k in eachindex(tspan) ]

    xL = [p[1] for p in left_end]
    yL = [p[2] for p in left_end]
    xR = [p[1] for p in right_end]
    yR = [p[2] for p in right_end]
end

# ╔═╡ 0fdbba6a-201e-4979-9338-b62314414bcb
begin
    ξ = range(-0.12, 0.12, length=200)
    track1_x = collect(ξ)
    track1_y = collect(ξ)
    track2_x = collect(ξ)
    track2_y = collect(-ξ)
...
begin
	plot(track1_x, track1_y, label="track y = x", xlabel="x (m)", ylabel="y (m)", aspect_ratio=:equal)
	plot!(track2_x, track2_y, label="track y = -x")
	plot!(x1_hist, y1_hist, linewidth=2, label="slider 1 center")
	plot!(x2_hist, y2_hist, linewidth=2, label="slider 2 center")
	plot!(x3_hist, y3_hist, linewidth=2, label="bar center")
end
...
gif(anim, "project_02_dual_slider_setup.gif", fps=20)

# ╔═╡ 37b9a94c-38b9-4428-b0df-f72a0c2bd6e7
md"""
### Interpretation of the motion

From the plots and animation, we can see that:
- the rigid bar rotates with the prescribed constant angular speed
- the sliders move along their diagonal tracks in a constrained and symmetric way
- the slider velocities and accelerations vary over time, even though the bar rotates at constant speed
- the animation confirms that the constraint equations are producing physically meaningful motion through one full rotation
"""
...
Starting from the velocity constraint

`C_q(q,t) q̇ + C_t(q,t) = 0`

we differentiate once more in time and solve for the generalized accelerations.

In this notebook, we will use the already-built functions:
- `Cq_mat(q,t)`
- `Ct_vec(q,t)`

and approximate the remaining second-derivative terms numerically.

That gives us a practical way to compute `q̈` without rebuilding the entire model from scratch.
"""

# ╔═╡ ae8136aa-04de-4e87-8185-926dbbf2cecd
md"""
### Generalized acceleration results

The matrix `qddot_hist` stores the acceleration history for all 9 generalized coordinates:

`[x₁̈, y₁̈, θ₁̈, x₂̈, y₂̈, θ₂̈, x₃̈, y₃̈, θ₃̈]`

We can now inspect selected components and compare them with the expected kinematic behavior of the mechanism.
"""
...
From these plots, we can compare the prescribed motion of the rigid bar to the constrained motion of the sliders.

Key observations:
- `θ₃̈` should remain close to zero because the bar rotates at constant angular speed
- the slider accelerations change over time because their motion is driven by the rotating-bar geometry
...
git-tree-sha1 = "20e89c795a2ad19c3ef426949c42cbe77dc32b84"
